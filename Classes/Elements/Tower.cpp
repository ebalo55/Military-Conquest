...
Tower::Tower(const sptr<sf::Font>& font, int life_point, double gold, sptr<Wave> wave_controller) :max_hp(life_point), hp(life_point), gold(gold), font(font),
    ...
    sprites["hud-bg"]->setPosition(sf::Vector2f{(float) Config::getWidth() / 2, 0});

    sf::Color gray = sf::Color(0xcc, 0xcc, 0xcc);

    sprites["heart"]->setPosition(sf::Vector2f{(float) Config::getWidth() / 2 + 10, 10});
    life_bar.setPosition(sf::Vector2f{(float) Config::getWidth() / 2 + 50, 15});
    life_bar.setFillColor(sf::Color(0x40, 0x99, 0x40));
    death_bar.setFillColor(sf::Color{0x99, 0x40, 0x40});
    life.setFont(*font);
    life.setFillColor(gray);
    life.setPosition(sf::Vector2f{(float) Config::getWidth() / 2 + 560, 15});
    life.setCharacterSize(12);

    sprites["coin"]->setPosition(sf::Vector2f{(float) Config::getWidth() / 2 + 10, 40});
    coin.setFont(*font);
    coin.setFillColor(gray);
    coin.setPosition(sf::Vector2f{(float) Config::getWidth() / 2 + 50, 40});
    coin.setCharacterSize(15);

    wave.setFont(*font);
    wave.setFillColor(gray);
    wave.setPosition(sf::Vector2f{(float) Config::getWidth() / 2 + 115, 40});
    wave.setCharacterSize(15);
    new_wave_countdown.setFont(*font);
    new_wave_countdown.setFillColor(sf::Color(0xd4, 0xe7, 0x68));
    new_wave_countdown.setPosition(sf::Vector2f{(float) Config::getWidth() / 2 + 215, 40});
    new_wave_countdown.setCharacterSize(16);
    bosses.setFont(*font);
    bosses.setFillColor(sf::Color(0xff, 0x55, 0x55));
    bosses.setPosition(sf::Vector2f{(float) Config::getWidth() / 2 + 550, 40});
    bosses.setStyle(sf::Text::Style::Bold);
    bosses.setCharacterSize(15);

    time.setFont(*font);
    time.setFillColor(gray);
    time.setPosition(sf::Vector2f{(float) Config::getWidth() / 2 + 425, 40});
...
void Tower::syncStats() {
    ...
    death_bar.setPosition(sf::Vector2f{(float) (Config::getWidth() / 2 + 550 - lost_life_lenght), 15});
